...
int ui_loop(int argc, char **argv, const char *name)
{
  ...
  vi = fp::glXChooseVisual(dpy, DefaultScreen(dpy), attributeList);
  if (vi == NULL) {
  ...
  cx = fp::glXCreateContext(dpy, vi, 0, GL_TRUE);
  assert(cx);
  ...
  assert(fp::glXMakeCurrent(dpy, win, cx));
